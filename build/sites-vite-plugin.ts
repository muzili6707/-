  ...
  return {
    ...
    async closeBundle() {
      ...
      const workerDirectory = entries.find(
        (entry) => entry.isDirectory() && !["client", "server", ".openai"].includes(entry.name),
      );
      const workerEntry = workerDirectory && resolve(distDirectory, workerDirectory.name, "index.js");
      if (workerEntry && await exists(workerEntry)) {
